...
def staff_cargo_rqst():
	...
	q="select * from bookings inner join customers using(customer_id) inner join prices using(price_id) where (from_branch='%s' or to_branch='%s') and (booking_status!='canceled') order by booking_id desc"%(brid,brid)
	res=select(q)
	data['booking']=res
	print(res)

...
def dboy_view_pickups():
	...
	dname=session['dname']
	did=session['did']
	q="select * from bookings inner join customers using(customer_id) where from_branch in (select branch_id from deliveryboys where boy_id='%s') and booking_status in('payed','pickedup')"%(did)
	res=select(q)
	print(q)
	data['works']=res
	print(res)

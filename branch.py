...
def branch_manage_staffs():
	...
	if action=='retired':
		...
		q="update login set user_type='retired' where username='%s'"%(id)
		update(q)
		return redirect(url_for('branch.branch_manage_staffs'))
	if action=='delete':
...
def branch_view_bookings():
	...
	res=select(q)
	data['booking']=res
	q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and (booking_status !='pending' AND booking_status!='canceled') "%(bid,bid)
	print(q)
	...
	if 'submit' in request.form:
		...
		if ty=='all':
			if from_date:
				...
				data['booking']=res
				print(res)
				q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and ( booking_date between '%s' and '%s') and (booking_status !='pending') and (booking_status !='pending' AND booking_status!='canceled')"%(bid,bid,from_date,to_date)
				res=select(q)
				data['total']=res[0]['sum']
				session['sum']=data['total']
				print(data['total'])
				...
				session['mains']=q
				print(res)
				q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and  (booking_status !='pending') and (booking_status !='pending' AND booking_status!='canceled')"%(bid,bid)
				res=select(q)
				data['total']=res[0]['sum']
				session['sum']=data['total']
				print(data['total'])
			...
			if from_date:
				q="select * from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and( booking_date between '%s' and '%s') and (booking_status !='pending' AND booking_status!='canceled')"%(bid,bid,from_date,to_date)
				print(q)
				res=select(q)
				data['booking']=res
				session['mains']=q
				...
				q="select * from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and (booking_status !='pending' AND booking_status!='canceled')"%(bid,bid)
				print(q)
				res=select(q)
				data['booking']=res
				print(res)

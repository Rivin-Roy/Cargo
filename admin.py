...
def admin_manage_cargo_prices():
	...
	if 'submit' in request.form:
		...
		q="insert into prices values(NULL,'%s','%s','%s','%s','%s','active')"%(maxkg,maxh,maxw,maxkm,maxp)
		insert(q)
	...
	if 'action' in request.args:
		...
		action=None
	if action=='active':
		q="update prices set pstatus='active' where price_id='%s'"%(id)
		update(q)
		return redirect(url_for('admin.admin_manage_cargo_prices'))
	if action=='inactive':
		q="update prices set pstatus='inactive' where price_id='%s'"%(id)
		update(q)
		return redirect(url_for('admin.admin_manage_cargo_prices'))
	if action=='delete':
...
def admin_view_booking():
	...
	res=select(q)
	data['booking']=res
	q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where  (booking_status !='pending' AND booking_status!='canceled') "
	res=select(q)
		...
		if ty=='all':
			if from_date:		
				if branch =='no':
					...
					data['booking']=res
					print(res)
					q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where ( booking_date between '%s' and '%s') and (booking_status !='pending' AND booking_status!='canceled')"%(from_date,to_date)
					res=select(q)
					data['total']=res[0]['sum']
					session['sum']=data['total']
					print(data['total'])
				else:
					q="select * from bookings inner join customers using(customer_id) where( booking_date between '%s' and '%s') and (from_branch='%s' or to_branch='%s')"%(from_date,to_date,branch,branch)
					print(q)
					session['mains']=q
					res=select(q)
					data['booking']=res
					print(res)
					q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where ( booking_date between '%s' and '%s') and (booking_status !='pending' AND booking_status!='canceled') and (from_branch='%s' or to_branch='%s')"%(from_date,to_date,branch,branch)
					res=select(q)
					data['total']=res[0]['sum']
					print(data['total'])
					session['sum']=data['total']
				...
				if branch =='no':
					...
					q="select * from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s')"%(branch,branch)
					print(q)
					res=select(q)
					data['booking']=res
					session['mains']=q
					...
					print(res)
					q="select *,sum(amount) as sum from bookings inner join customers using(customer_id) where (booking_status !='pending' AND booking_status!='canceled') and (from_branch='%s' or to_branch='%s')"%(branch,branch)
					res=select(q)
					data['total']=res[0]['sum']
					print(data['total'])
					session['sum']=data['total']
			...
			if from_date:		
				if branch =='no':
					...
					q="select * from bookings inner join customers using(customer_id) where( booking_date between '%s' and '%s') and (booking_status !='pending' AND booking_status!='canceled')"%(from_date,to_date)
					print(q)
					session['mains']=q
					res=select(q)
					data['booking']=res
					...
					q="select * from bookings inner join customers using(customer_id) where( booking_date between '%s' and '%s') and (from_branch='%s' or to_branch='%s') and (booking_status !='pending' AND booking_status!='canceled')"%(from_date,to_date,branch,branch)
					print(q)
					session['mains']=q
					res=select(q)
					data['booking']=res
				...
				if branch =='no':
					...
					q="select * from bookings inner join customers using(customer_id) where (booking_status !='pending' AND booking_status!='canceled')"
					print(q)
					session['mains']=q
					res=select(q)
					data['booking']=res
					...
					q="select * from bookings inner join customers using(customer_id) where (from_branch='%s' or to_branch='%s') and (booking_status !='pending' AND booking_status!='canceled')"%(branch,branch)
					print(q)
					session['mains']=q
					res=select(q)
					data['booking']=res

...
def customer_view_bookings():
	...
	if 'action1' in request.args:
		...
		boid=request.args['boid']	
		q="update bookings set booking_status='canceled' where booking_id='%s'"%(boid)
		update(q)
	...
	if 'action2' in request.args:
		...
		q="update bookings set booking_status='canceled' where booking_id='%s'"%(boid)
		update(q)
		flash("booking canceled and amount "+amt +" will be refund with in few days")
		return redirect(url_for('customer.customer_view_bookings'))
...
def customer_track_cargo():
	...
	data['branch']=res
	print(res)
	q="select * from bookings inner join branches ON(branches.branch_id=bookings.from_branch) inner join payment using(booking_id) where customer_id='%s' and booking_status!='canceled' order by branch_id desc"%(cid)
	res=select(q)
	data['bookings']=res
	print(res)
	if 'action' in request.args:

...
def login():
	...
	if 'submit' in request.form:
		...
		if res:
			...
			if res[0]['user_type']=='retired':
				flash("YOU ARE RETIRED")
				return redirect(url_for('public.login'))
...
def customerreg():
	...
	if 'submit' in request.form:
		...
		if cpassword==password:
			...
			flash("PASSWORD AND CONFORM PASSWORD DOES NOT MATCH")
	...
	if 'submit' in request.form:	
		...
		if res:
			...
			try:
				...
				gmail.login('clinstonclintu@gmail.com','123')
			except Exception as e:
				print("Couldn't setup email!!"+str(e))

			msg = MIMEText(msg)

			msg['Subject'] = 'OTP FOR PASSWORD RECOVRY Q Cargo'

			msg['To'] = email

			msg['From'] = 'clinstonclintu@gmail.com'

			try:
				gmail.send_message(msg)
				print(msg)
				flash("e-mail successfully sent to " +email)
				session['rd']=rd
				session['email']=email
				# data['email']=email
				...
				print("ERROR!!! Couldn't send OTP to " +email, str(e))
